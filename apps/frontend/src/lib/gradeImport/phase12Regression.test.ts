...
describe("phase 12 grade import regression suite", () => {
  ...
  it("guards unified contextual import policy in source until a component test harness is added", () => {
    ...
    expect(gradeInputCellSource).toContain("font-black");
    ...
    expect(gradeInputCellSource).toContain("text-yellow-600");
    ...
    expect(pwaManagerSource).toContain("UPDATE_LOCK_KEY");
    ...
    expect(packageSource).toContain('"viewport:sync": "node scripts/sync-viewport-observations.mjs"');
    expect(viewportSyncSource).toContain(".from(\"viewport_observations\")");
    expect(viewportSyncSource).toContain("SUPABASE_SERVICE_ROLE_KEY");
    expect(viewportSyncSource).toContain(".codex/viewport-observations/latest.json");
