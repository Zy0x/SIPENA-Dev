import { buildExecutableImportOperations, type ExecutableImportOperation, type ExecutableImportPlan, type GradeOperation, type GradeTarget, type ImportPlan } from "@/lib/gradeImport";
...
function decisionAction(operation: GradeOperation, executableOperation?: ExecutableImportOperation): ImportDecisionAction {
  if (executableOperation && operation.isAutoConverted && executableOperation.action === "fill_empty") return "convert";
  if (executableOperation) return executableOperation.action === "overwrite" ? "overwrite" : "save";
...
function decisionStatus(operation: GradeOperation, executableOperation?: ExecutableImportOperation): ImportDecisionStatus {
  if (executableOperation) return operation.warnings.length ? "auto_fixed" : "safe";
...
function decisionReason(operation: GradeOperation, executableOperation?: ExecutableImportOperation): string {
  if (executableOperation?.action === "overwrite") {
    return "Nilai siap disimpan sebagai overwrite yang sudah lolos pemeriksaan.";
  }
  if (executableOperation && operation.isAutoConverted && operation.conversionLabel) return operation.conversionLabel;
  if (executableOperation) return "Nilai siap disimpan karena siswa, target, dan nilai sudah aman.";
...
export function buildImportDecisionGraph(
  ...
  const decisions = plan.gradeOperations.map<ImportDecision>((operation) => {
    ...
    const action = skipped ? "skip" : decisionAction(operation, executableOperation);
    const status = skipped ? "will_skip" : decisionStatus(operation, executableOperation);
    ...
    return {
      ...
      reason: skipped?.message || blocked?.message || decisionReason(operation, executableOperation),
      operation,
      executableOperation,
