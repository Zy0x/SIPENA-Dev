    ...
    if (exportFormat === "pdf") {
      ...
      if (hasSubjects) {
        ...
        const level2 = subjectsToUse.map((s) => ({
          content: s.name,
          styles: { halign: 'center', valign: 'middle', fillColor: [59, 130, 246], textColor: [255, 255, 255], fontSize: 7, overflow: 'linebreak' as const, cellPadding: { top: 1, right: 1, bottom: 1, left: 1 } },
        }));
        ...
        autoTable(doc, {
          head: [level1 as unknown as RowInput, level2 as unknown as RowInput],
          body: data.map((row) => Object.values(row)),
          startY: 32,
