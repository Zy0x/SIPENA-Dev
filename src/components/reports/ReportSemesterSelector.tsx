...
export type SemesterFilterValue = "1" | "2" | "all";

interface ReportSemesterSelectorProps {
...
export function ReportSemesterSelector({
  value,
  onChange,
  className,
  showIndicator = true,
}: ReportSemesterSelectorProps) {
  ...
  const options = [
    { value: "1" as const, label: "Semester 1", description: "Hanya data semester ganjil" },
    { value: "2" as const, label: "Semester 2", description: "Hanya data semester genap" },
    { value: "all" as const, label: "Semua Semester", description: "Gabungan data tahunan" },
  ];
...
/**
 * Hook untuk menggunakan semester filter di halaman laporan
 * Default mengikuti semester aktif global, tapi bisa di-override user
 */
export function useReportSemesterFilter() {
  const { activeSemesterNumber } = useAcademicYear();
  const [semesterFilter, setSemesterFilter] = useState<SemesterFilterValue>(
    activeSemesterNumber?.toString() as SemesterFilterValue || "all"
  );

  // Sync dengan semester aktif saat pertama kali mount
  useEffect(() => {
    if (activeSemesterNumber) {
      setSemesterFilter(activeSemesterNumber.toString() as SemesterFilterValue);
    }
...
export default ReportSemesterSelector;
